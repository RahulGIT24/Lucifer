...
import { setChatSessionId, setCurrentPrompt } from "@/lib/store/features/chat/chatSlice";
...
const Sidebar = () => {
  ...
  return (
    <div
      className={`relative top-0 left-0 h-screen bg-zinc-950 transition-transform duration-300 ease-in-out lg:w-1/5 transform md:relative md:translate-x-0 md:w-2/5 ${
    ...
    >
      ...
      <div
        className={`sm:${showSideBar ? "block" : "hidden"} lg:block md:block`}
      >
        {user && (
          <Button
            variant="secondary"
      ...
      <div className="absolute bottom-0 flex flex-col justify-end w-full">
        {user && (
          <Button
            variant="destructive"
            disabled={isLogout}
            className={`md:block hidden lg:block sm:${
